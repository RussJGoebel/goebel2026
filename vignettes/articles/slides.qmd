---
title: "Area-to-Area Spatial Prediction"
subtitle: "fastblm and spatintegrate"
...
target_proj    <- spatintegrate::ensure_projected(goebel2026::target_grid_square)
target_bbox    <- as.numeric(sf::st_bbox(target_proj))
delta          <- sqrt(as.numeric(sf::st_area(target_proj[1,])))
Lx <- target_bbox[3] - target_bbox[1]   # domain width  in metres
Ly <- target_bbox[4] - target_bbox[2]   # domain height in metres
```
...
```{r slide1, echo=FALSE, fig.height=4.5, fig.width=11}
p_obs <- ggplot() +
  geom_sf(data = target_proj, fill = "grey92", colour = NA) +
  geom_sf(data = soundings_proj, aes(fill = SIF_757nm),
          colour = NA, alpha = 0.7) +
  scale_fill_distiller(palette = "YlOrRd", direction = 1,
                       name = "SIF", na.value = "grey80") +
  labs(title = "OCO-2 SIF footprints\n(colored by observed SIF)") +
  theme_void(base_size = 11) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))

p_grid <- ggplot() +
  geom_sf(data = target_proj, fill = "steelblue", colour = "white",
          linewidth = 0.1, alpha = 0.6) +
  labs(title = "Target grid\nWhat is the SIF at each pixel?") +
  theme_void(base_size = 11) +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))

p_obs + plot_spacer() + p_grid +
...
The posterior mean $\hat{x} = \mathbb{E}[x\mid y]$ is the optimal linear predictor. See [*Kriging is Bayesian linear regression*](../posts/kriging-gp-equivalence/index.qmd).
...
[Heaton et al. (2019)](https://doi.org/10.1007/s13253-018-00348-w) compared **12 leading methods** on 105,000 satellite surface temperature observations. See  [*my writeup*](../posts/kriging-competition/index.Rmd) of these methods.
...
```{r spatintegrate-intro, eval=FALSE}
...
```{r worked-plot-sar, echo=FALSE, fig.height=4.5}
plot_sf          <- target_proj
plot_sf$pred_sar <- pred_sar
plot_sf$se_sar   <- se_sar

p1 <- ggplot(plot_sf) +
  geom_sf(aes(fill = pred_sar), colour = NA) +
  scale_fill_viridis_c(name = "SIF") +
  labs(title = "SAR posterior mean") + theme_void(base_size = 10)

p2 <- ggplot(plot_sf) +
  geom_sf(aes(fill = se_sar), colour = NA) +
  scale_fill_viridis_c(name = "SE", na.value = "grey90") +
  labs(title = "SAR posterior SE") + theme_void(base_size = 10)
...
```{r plot-final, echo=FALSE, fig.height=4.5}
plot_sf$pred_fourier <- pred_fourier

p1 <- ggplot(plot_sf) +
  geom_sf(aes(fill = pred_sar), colour = NA) +
  scale_fill_viridis_c(name = "SIF") +
  labs(title = sprintf("Raster SAR  (p = %d pixels)", p)) +
  theme_void(base_size = 10)

p2 <- ggplot(plot_sf) +
  geom_sf(aes(fill = pred_fourier), colour = NA) +
  scale_fill_viridis_c(name = "SIF") +
  labs(title = sprintf("Fourier-Matérn  (K = %d modes)", K)) +
  theme_void(base_size = 10)
