## Fit downscaling model on OCO-2 SIF data using REML
##
## Steps:
## 1. Compute aggregation matrix A
## 2. Precompute W and Q at fixed rho
## 3. Use tune_reml to select phi via REML marginal likelihood
...
message("Starting REML tuning...")
tuned <- fastblm::tune_reml(
  y             = y,
  A             = A,
  Q_fun         = Q_fun,
...
message(sprintf("REML ll: %.4f", tuned$value))
