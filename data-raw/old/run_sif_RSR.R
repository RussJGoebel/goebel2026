# data-raw/run_sif_rsr.R
#
# Main SIF downscaling run with hard RSR.
#
# Model:
#   y = A_aug * gamma + epsilon,  gamma = (r, beta)
#   r ~ SAR(rho),  beta ~ N(0, lambda_beta^{-1} I)
#   Hard RSR constraint: C_fold * gamma = 0  where
#     C_fold = t(X_obs_water[train,]) %*% A_train
#
# rho tuned via 1D CV golden section, phi profiled at each rho.
...
if (!.should_skip("results_sif_rsr")) {
  message("\n== SIF RSR: rho CV-tuned, hard RSR constraint, PCG solver ==")
  ...
  usethis::use_data(results_sif_rsr, overwrite = TRUE)
