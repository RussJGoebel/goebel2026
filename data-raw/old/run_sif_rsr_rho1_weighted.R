# data-raw/run_sif_rsr_rho1_rinv.R
#
# SIF downscaling run with hard RSR, rho=1, heteroskedastic noise weights.
#
# Model:
#   y = A_aug * gamma + epsilon,  gamma = (r, beta)
#   r ~ SAR(rho=1),  beta ~ N(0, lambda_beta^{-1} I)
#   epsilon_i ~ N(0, sigma_i^2)  where sigma_i = SIF_Uncertainty_757nm
#   Hard RSR constraint: C_fold * gamma = 0
#
# rho = 1 (fixed), phi profiled via CV golden section.
# R_inv = diag(1 / sigma_eps^2) passed to tune_cv and fit_fastblm.
...
message(sprintf("Noise weights: min_sigma=%.4f  max_sigma=%.4f  median_sigma=%.4f",
                min(sigma_eps), max(sigma_eps), median(sigma_eps)))
...
t_check <- system.time({
  # Sanity check: single fold manually to verify R_inv path
  fold      <- 1L
  train_idx <- which(fold_assignments != fold)
  test_idx  <- which(fold_assignments == fold)
  fit_chk   <- fastblm::fit_fastblm(
    y_sif[train_idx], A_aug[train_idx, ], prior_check$Q,
    phi    = 100,
    R_inv  = R_inv[train_idx, train_idx],
    solver = "cholesky"
  )
  fit_chk   <- fastblm::constrain(fit_chk, fold_C_list[[fold]])
  y_pred    <- as.numeric(A_aug[test_idx, ] %*% fit_chk$posterior_mean)
  mse_chk   <- mean((y_sif[test_idx] - y_pred)^2)
  message(sprintf("  fold 1 check: mse=%.4e", mse_chk))
})
...
if (!.should_skip("results_sif_rsr_rho1_rinv")) {
  message("\n== SIF RSR: rho=1, hard RSR, heteroskedastic R_inv ==")
  ...
  results_sif_rsr_rho1_rinv <- list(
    run_name              = "sif_rsr_rho1_rinv",
    tags                  = list(tuning     = "cv",
                                 response   = "SIF_757nm",
                                 covariates = "water",
                                 constraint = "hard_RSR",
  ...
  usethis::use_data(results_sif_rsr_rho1_rinv, overwrite = TRUE)
...
message("\nSIF RSR rho=1 R_inv results saved to data/.")
