...
fine_grid_buffered <- spatintegrate::ensure_projected(goebel2026::target_grid_150m)
res_m              <- 150
...
make_W_from_nb <- function(nb) {
  ...
  i <- rep(seq_along(lw$neighbours), lengths(lw$neighbours))
  j <- unlist(lw$neighbours, use.names = FALSE)
  x <- unlist(lw$weights, use.names = FALSE)

  Matrix::sparseMatrix(
    i = i,
    j = j,
    x = x,
    dims = c(length(lw$neighbours), length(lw$neighbours))
  )
}
...
usethis::use_data(setup_shared_150m, overwrite = TRUE)
usethis::use_data(setup_albedo_150m, overwrite = TRUE)
usethis::use_data(setup_sif_150m,    overwrite = TRUE)
