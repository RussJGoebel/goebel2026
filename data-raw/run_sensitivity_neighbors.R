...
tab_nb_alb <- data.frame(
  Neighbourhood = c("Queen (baseline)", "Rook",
                    "LC-aware ($\\varepsilon=0.1$)",
                    "LC-aware ($\\alpha=0$)"),
...
```{r nb_sif}
nbs_sif <- list(r$sif, r$sif_nb_rook, r$sif_nb_lc_eps, r$sif_nb_lc_alpha)
...
functions parameterised by a width $\tau$: as $\tau \to 0$ the response
becomes increasingly peaked at the footprint centre, approaching the centroid
