...
message(sprintf("  Soundings: %d", nrow(soundings_proj)))
...
# Remove outlier: single sounding with SIF = 4.91 (quality flag = 1,
# physically implausible -- no vegetated surface produces SIF this large
# at 757nm). Sensitivity analysis confirms removing it shifts the posterior
# mean by <= 0.20 SIF units locally and < 0.001 on average across the domain.
outlier_idx    <- which.max(y_sif)
keep_idx       <- setdiff(seq_along(y_sif), outlier_idx)
message(sprintf("  Removing outlier sounding %d: SIF = %.3f",
                outlier_idx, y_sif[outlier_idx]))
